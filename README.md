...
進みます。必ずまず「転換中」になり、演目に**転換用プレイリスト**が割り当てられて
いればそのBGMを順番に再生し(最後まで流れたら先頭に戻ってループ)、割り当てが
無ければ**無音のまま**転換中になります(いきなり上演中へスキップしません)。
もう一度 `N` を押すと「上演中」に切り替わります。
...
リハーサルをした後、その続き位置(同じプレイリストの再開位置・戻る履歴)を
本番に持ち越したくない場合は、`control.html` の「↺ 最初からにリセット」ボタン
(`POST /program/reset`)で `main.py` を再起動せずに進行状態を開始前に戻せます。
...
`http://127.0.0.1:4000` にアクセスすると管理画面が開きます
(`localhost`表記だと、環境によって名前解決が遅くDNS由来の遅延が出ることがあるため`127.0.0.1`を推奨)。
...
- 転換用プレイリスト: 名前を付けたプレイリスト(複数曲・並べ替え可)を作成できます
  ...
  既定では最後まで流れたら先頭に戻ってループしますが、プレイリストごとの
  「最後まで流れたら繰り返す」チェックボックスをオフにすると、1回流れ切った
  ところで無音のまま止まります(歌のインストを流し切ったら止めたい場合など)。
