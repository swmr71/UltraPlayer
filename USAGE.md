...
する。ただし**上演中BGMの切り替えだけはフェード無し**(即切り替え)にしている
(`ProgramController` が `mode == "performing"` のときは `fade_ms=0` を渡す)。
`pygame.mixer.music` は音声ストリームを1本しか持てず本当のクロスフェード(2曲を
重ねて鳴らす)はできないため、実際には「今の曲をフェードアウトして無音にしてから
次の曲をフェードインする」疑似クロスフェードになっている。フェードアウトは
