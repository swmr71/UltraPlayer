...
function layoutVideoSide(data) {
  ...
  if (video.syncPlayback) {
    ...
    // 一時停止中はBGMの経過秒数も止まっているので追跡する意味が無い
    // (むしろ経過時間との差分だけが増えてシーク扱いされてしまう)。
    // 再生中だけ追跡し、一時停止したら基準をリセットしておく。
    if (data.playing) {
...
let lastKnownElapsed = null;
let lastElapsedAt = null;

function resetVideoSyncTracking() {
  lastKnownElapsed = null;
  lastElapsedAt = null;
}

function seekVideoTo(elapsedSec) {
...
function syncVideoPosition(elapsedSec) {
  if (typeof elapsedSec !== "number" || !Number.isFinite(elapsedSec)) return;
  const now = performance.now();
  if (lastKnownElapsed != null && lastElapsedAt != null) {
    const wallDeltaSec = (now - lastElapsedAt) / 1000;
    const bgmDeltaSec = elapsedSec - lastKnownElapsed;
    if (Math.abs(bgmDeltaSec - wallDeltaSec) > VIDEO_SEEK_JUMP_SEC) {
      seekVideoTo(elapsedSec);
    }
  }
  lastKnownElapsed = elapsedSec;
  lastElapsedAt = now;
}
